...
def render_voice_input() -> None:
    """
    Renders a reliable mic button using Web Speech API.
    When user clicks 'Use', it writes the spoken text directly
    into the Streamlit topic input box using JS DOM manipulation.

    Works on Chrome and Edge only.
    No return value needed — it directly fills the input box.
    """
    ...
    // ── Fill Streamlit input box ────────────────────────────────────────────
    // Streamlit renders text inputs as <input> inside iframes/shadow DOM.
    // We walk up to the parent window and find the topic input by placeholder.
    function useTranscript() {
        if (!finalText.trim()) return;

        var text = finalText.trim();

        // ── Method 1: postMessage to parent (works when in iframe) ─────────
        try {
        ...
        // ── Method 2: find input in parent DOM directly ────────────────────
        try {
            var inputs = window.parent.document.querySelectorAll('input[type="text"]');
            for (var i = 0; i < inputs.length; i++) {
                var placeholder = inputs[i].placeholder || "";
                if (placeholder.toLowerCase().includes("quantum") ||
                    placeholder.toLowerCase().includes("topic")   ||
                    placeholder.toLowerCase().includes("e.g")     ||
                    inputs[i].id === "topic_input") {

                    // React / Streamlit needs a native input event
    ...
    components.html(voice_html, height=75, scrolling=False)


def inject_voice_listener() -> None:
    """
    Injects a JS listener into the Streamlit page that catches
    the SET_VOICE_TOPIC postMessage and fills the input box.
